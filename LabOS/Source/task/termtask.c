...
#define comRX_BLOCK_TIME			( ( portTickType ) 0xff )
...
void vStartTermTask( unsigned portBASE_TYPE uxPriority, unsigned long ulBaudRate, unsigned portBASE_TYPE uxLED )
{
...
{
signed char cByteKey, cByteRxed;
...
//		for( cExpectedByte = comFIRST_BYTE; cExpectedByte <= comLAST_BYTE; cExpectedByte++ )
		{
			...
			available. */
			if( xSerialGetChar( xPort, &cByteRxed, comNO_BLOCK ) )
			{
//				CommandPutChar(cByteRxed);
				DisplayCmd(cByteRxed);
			...
			if( xKeyGetChar(&cByteKey ) )
			{
//				CommandPutChar(cByteRxed);
				DisplayCmd(cByteRxed);

				// TODO if echo enable
				if( xSerialPutChar( xPort, cByteKey, comNO_BLOCK ) == pdPASS )
				{
