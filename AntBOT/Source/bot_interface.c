...
void Motor_IO_Config(void){
	MOTOR_CONTROL_DDR	|= (1<<L_MOTOR_CNTRL)|(1<<R_MOTOR_CNTRL)|(1<<C_MOTOR_CNTRL);
	MOTOR_CONTROL_PORT	&=~(1<<L_MOTOR_CNTRL)|(1<<R_MOTOR_CNTRL)|(1<<C_MOTOR_CNTRL);
	MOTOR_FEEDBACK_DDR	&=~(1<<L_MOTOR_FB)|(R_MOTOR_FB);
}

void Buttons_IO_Config(void){
	BUTTON_PORT	|= (1<<BUTTON1)|(1<<BUTTON2)|(1<<BUTTON3)|(1<<BUTTON4);
}

void Bumpers_Ref_IO_Config(void){
	BUMPER_REF_DDR	|=	 (1<<BUMPER_REF);
	BUMPER_REF_PORT	&=	~(1<<BUMPER_REF);
	BUMPER_REF_OCR	=	0;
}

void Wall_Sensor_En_IO_Config(void){
	WALL_SENSOR_EN_DDR	|=	 (1<<B_WALL_SENSOR_EN)|(1<<F_WALL_SENSOR_EN);
	WALL_SENSOR_EN_PORT	&=	~(1<<B_WALL_SENSOR_EN)|(1<<F_WALL_SENSOR_EN);
}

void Edge_Sensor_En_IO_Config(void){
	EDGE_SENSOR_EN_DDR	|=	 (1<<EDGE_SENSOR_EN);
	EDGE_SENSOR_IN_PORT	&=	~(1<<EDGE_SENSOR_EN);
}

void Line_Sensor_En_IO_Config(void){
	LINE_SENSOR_EN_DDR	|=   (1<<LINE_SENSOR_EN);
	LINE_SENSOR_EN_PORT	&=	~(1<<LINE_SENSOR_EN);
}
